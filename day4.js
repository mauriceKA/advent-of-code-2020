...
var fieldValidators = {
    "byr": (val) => { return val.match(/\d{4}/) && val >= "1920" && val <= "2002"; },
    "iyr": (val) => { return val.match(/\d{4}/) && val >= "2010" && val <= "2020"; },
    "eyr": (val) => { return val.match(/\d{4}/) && val >= "2020" && val <= "2030"; },
    "hgt": (val) => { 
        var amount, unit, valid = false;
        var matches = val.match(/^(\d+)(cm|in)$/);
        if (matches) {
            [, amount, unit] = matches;
            if (unit === "cm") {
                valid = parseInt(amount) >= 150 && parseInt(amount) <= 193;
            } else if (unit === "in") {
                valid = parseInt(amount) >= 59 && parseInt(amount) <= 76;
            }
        }
        return valid;
    },
    "hcl": (val) => { return val.match(/\#[0-9a-f]{6}$/); },
    "ecl": (val) => { return ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].indexOf(val) >= 0; },
    "pid": (val) => { return val.match(/^\d{9}$/); }
};
var reqFieldNames = Object.keys(fieldValidators);


// part 1
var numberOfValidPassports = passports.map( passport => { 
    var fieldStrings = passport.split(/[\n ]/);
    if (fieldStrings.length < 7) return 0; // just an optimization
    var fieldNames = fieldStrings.map( field => { return field.match(/^(...)/)[1]; } );
    // use passport invalid and some for efficiency: some stops at the first true
    var passportInvalid = reqFieldNames.some( reqFieldName => { 
        // return true if invalid
        return fieldNames.indexOf(reqFieldName) < 0; 
    });
    return passportInvalid ? 0 : 1;
...
var numberOfValidPassports = passports.map( passport => { 
    ...
    if (fieldStrings.length < 7) return 0; // just an optimization
    var fieldsObject = {};
    fieldStrings.forEach( field => { 
        var matches = field.match(/^(...):(.*)$/); 
        fieldsObject[matches[1]] = matches[2];  
    });
    // use passport invalid and some for efficiency: some stops at the first true
    var passportInvalid = reqFieldNames.some( reqFieldName => { 
        // console.log(reqFieldName + " invalid:" + (fieldsObject[reqFieldName] === undefined || !fieldValidators[reqFieldName](fieldsObject[reqFieldName])));
        // return true if invalid
        return fieldsObject[reqFieldName] === undefined || !fieldValidators[reqFieldName](fieldsObject[reqFieldName]);
    });
    // console.log(passport + " invalid:" + passportInvalid + "\n----------\n\n");
    return passportInvalid ? 0 : 1;
}).reduce( (acc, val) => acc + val );
console.log(numberOfValidPassports);
