var input = `light chartreuse bags contain 1 mirrored yellow bag, 2 vibrant violet bags.
...
var bagSpecs = input.split("\n").map(rule => {
    var predecessor, successor;
    [predecessor, successorList] = rule.split(" contain ");
    var successors = successorList.substr(0,successorList.length-1).split(", ").map(succ => {
        var amount, color;
        if (succ === "no other bags") {
            amount = 0;
        } else {
            [, amount, color] = succ.match(/^(\d+) (.*?) bags?$/);
        }
        return {
    ...
    return {
        bagColor: predecessor.match(/^(.*) bags/)[1],
...
function getNumberOfContainedBags(color) {
    var rule = bagSpecs.find( spec => { return spec.bagColor === color} );
    return rule.successors.map( succ => {
        return succ.amount > 0 ? (succ.amount + succ.amount * getNumberOfContainedBags(succ.bagColor)) : 0;
    }).reduce( (acc, val) => { return acc + val; } );
}
console.log(getNumberOfContainedBags("shiny gold"));