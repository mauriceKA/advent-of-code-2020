...
var numberOfValidPasswords = input.split("\n").filter( line => { return line.length > 0; } ).map( line => {
    ...
    var chacacterCount = password.split("").map( char => { return char === character ? 1 : 0; } ).reduce( reducer );
    //console.log(min, max, character, password, chacacterCount);
    return (chacacterCount >= min && chacacterCount <= max) ? 1 : 0;
}).reduce( reducer );
