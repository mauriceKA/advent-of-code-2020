...
var numberOfValidPasswords = input.split("\n").map( line => {
    ...
    var characterCount = password.split("").map( char => { return char === character ? 1 : 0; } ).reduce( reducer );
    //console.log(min, max, character, password, characterCount);
    return (characterCount >= min && characterCount <= max) ? 1 : 0;
}).reduce( reducer );
...
var numberOfValidPasswords = input.split("\n").map( line => {
    ...
    var characterCount = ( (password.charAt(pos1-1) === character) ? 1 : 0) + ( (password.charAt(pos2-1) === character) ? 1 : 0);
    //console.log(pos1, pos2, character, password, "pos1:"+password.charAt(pos1-1), "pos2:"+password.charAt(pos2-1), characterCount);
    return (characterCount === 1) ? 1 : 0;
}).reduce( reducer );
console.log(numberOfValidPasswords);
